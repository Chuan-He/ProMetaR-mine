...
class VNet(nn.Module):
    ...
    def forward(self, gradients1, gradients2, gradients3, param_name):
        ...
        if gradients2 == None:
            ...
            #gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1).float())).repeat_interleave(4,-1).reshape(d_1,d_2).half()
            #changed_gradients = (gradients1 + gradients3) * gamma_t
            a_grad = gradients1
            b_grad = gradients3
            ...
            #gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1).float())).repeat_interleave(2,-1).reshape(d_1,d_2).half()
            # gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1).float())).repeat_interleave(4,-1).reshape(d_1,d_2).half()
            # changed_gradients = (gradients1 + gradients2) * gamma_t
            a_grad = gradients1
            b_grad = gradients2
        else:
            raise NotImplemented
        ...
        if torch.dot(a_grad_norm.flatten(), b_grad_norm.flatten()) < 0:
            changed_gradients = a_grad - lamda * torch.dot(
...
class ProMetaR(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        for t in range(1):
            ...
            index_set = list(range(batch_size))
            random.shuffle(index_set)

            N = 2
            l = len(index_set)
            m = (l-1) // N + 1
            index_set_list = [index_set[i*m:(i+1)*m] for i in range(N)]
            ...
            new_label_list = [torch.tensor([label_lst[j] for j in index_set_list[i]]).cuda() for i in range(N)]
            new_image_list = [image[index_set_list[i]].cuda() for i in range(N)]

            step_loss = []
            for i in range(1):
  
                task_model = maml.clone(allow_nograd=True)
                adaptation_logits_ce, adaptation_rag_text, adaptation_rag_image = task_model(image=new_image_list[i])
                adaptation_loss_ce = F.cross_entropy(adaptation_logits_ce, new_label_list[i])
                ...
                query_logits_ce, query_rag_text, query_rag_image= task_model(image=new_image_list[(i+1)%2])
                adapt_loss = F.cross_entropy(query_logits_ce, new_label_list[(i+1)%2]) + query_rag_text + query_rag_image
        ...
        loss_summary = {
            "loss": step_loss.item(),
            #"acc": compute_accuracy(query_logits_ce, label)[0].item(),
        }
