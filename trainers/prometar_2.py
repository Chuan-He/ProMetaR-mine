...
def gradient_update_2(model, loss1, loss2, grad_func=None):
    diff_params = [p for p in model.parameters() if p.requires_grad]
...
class VNet(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.linear_vision_gamma = nn.Sequential(nn.Linear(vision_ctx_dim*4, vision_ctx_dim*2), nn.Sigmoid(), nn.Linear(vision_ctx_dim*2, vision_ctx_dim*2))
        self.linear_text_gamma = nn.Sequential(nn.Linear(text_ctx_dim*4, text_ctx_dim*2), nn.Sigmoid(), nn.Linear(text_ctx_dim*2, text_ctx_dim*2))       
    ...
    def forward(self, gradients1, gradients2, gradients3, param_name):
        ...
        if gradients2 == None:
            ...
            gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1).float())).reshape(d_1,d_2).half()
            changed_gradients = gamma_t * (gradients1 + gradients3)
            
        elif gradients3 == None:
            input_gradients = torch.cat((gradients1, gradients2), 0)
            #gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1).float())).repeat_interleave(2,-1).reshape(d_1,d_2).half()
            gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1).float())).reshape(d_1,d_2).half()
            changed_gradients = gamma_t * (gradients1 + gradients2)
    ...
    def forward_2(self, gradients1, gradients2, param_name):
        ...
        input_gradients = torch.cat((gradients1, gradients2), 1)
        gamma_t = torch.sigmoid(linear_gamma(input_gradients.float())).half()
        changed_gradients = gradients1 +  gamma_t * gradients2
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label_idx=None):
        ...
        logits_i = logit_scale * (image_features @ text_features_zs.half().t())
        ...
        sim_text = cos(text_features, text_features_zs)
...
class ProMetaR(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        while t<8:
            ...
            step_loss = []
            for i in range(N):
                image_rest = torch.cat([image_li[j] for j in range(N) if j != i])
                label_rest = torch.cat([label_li[j] for j in range(N) if j != i])
                cls_rest = [item for j in range(N) if j != i for item in cls_li[j]]
                
                if len(image_li[i]) == 0 or len(image_rest) == 0:
                    break
  
                task_model = maml.clone(allow_nograd=True)
                adaptation_logits_ce, adaptation_rag_text, adaptation_rag_image = task_model(image=image_rest, label_idx=cls_rest)
                adaptation_loss_ce = F.cross_entropy(adaptation_logits_ce, label_rest)
                
                task_model.adapt_2(adaptation_loss_ce, adaptation_rag_image, allow_nograd=True, grad_func=self.vnet, allow_unused=True)

                query_logits_ce, query_rag_text, query_rag_image= task_model(image=image_li[i], label_idx=cls_li[i])
                adapt_loss = F.cross_entropy(query_logits_ce, label_li[i]) + query_rag_text + query_rag_image

                step_loss.append(adapt_loss)

            if len(step_loss) != 0:
                step_loss = torch.mean(torch.stack(step_loss))
        ...
        # optim.zero_grad()
        # model = gradient_update_2(model, loss_ce, reg_text, grad_func=self.vnet)
        # optim.step()
