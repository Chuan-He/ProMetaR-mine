...
class VNet(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.linear_vision_gamma = nn.Sequential(nn.Linear(vision_ctx_dim*2*n_vis, vision_ctx_dim//2), nn.Sigmoid(), nn.Linear(vision_ctx_dim//2, vision_ctx_dim//2))
        self.linear_text_gamma = nn.Sequential(nn.Linear(text_ctx_dim*2*n_ctx, text_ctx_dim//2), nn.Sigmoid(), nn.Linear(text_ctx_dim//2, text_ctx_dim//2))       
