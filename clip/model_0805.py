...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, vision_prompts = None):
        ...
        if self.VPT_shallow:
            if vision_prompts == None:
                visual_ctx = self.VPT.expand(x.shape[0], -1, -1).half()
                x = torch.cat([x, visual_ctx], dim=1)
            else:
                visual_ctx = vision_prompts[0,:,:].expand(x.shape[0], -1, -1).half()
                x = torch.cat([x, visual_ctx], dim=1)
        else:
        ...
        if vision_prompts != None:
            layer_p, n_pro, vis_dim = vision_prompts.size()
            for i in range(self.layers):
                if 1 <= i < layer_p:
                    ctx = vision_prompts[i].expand(batch_size, -1, -1).permute(1, 0, 2)
                    prefix = x[:-n_pro, :, :]
